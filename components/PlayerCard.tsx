  ...
  return (
    <div className={`relative flex flex-col bg-[#0f172a]/90 rounded-xl border-2 transition-all duration-500 overflow-hidden ${isExpanded ? 'border-yellow-600 shadow-[0_0_30px_rgba(202,138,4,0.2)]' : 'border-slate-800 hover:border-slate-700 shadow-xl'}`}>
      ...
      <div className="p-4 flex flex-col gap-4">
        ...
        {/* 战力评估栏目 */}
        <div className="mt-1 flex flex-col gap-2 p-2.5 bg-slate-900/60 rounded-xl border border-slate-800 shadow-inner">
          <div className="flex items-center gap-2">
            <Sparkles className={`w-3.5 h-3.5 ${isAnalyzing ? 'text-purple-400 animate-spin' : 'text-yellow-500'}`} />
            <span className="text-[10px] font-black text-slate-400 tracking-[0.2em] uppercase">战力评估</span>
          </div>
          <span className={`text-[12px] font-black text-yellow-500 leading-tight block min-h-[1.5em] italic ${isAnalyzing ? 'animate-pulse opacity-50' : ''}`}>
            {player.aiEvaluation ? `「 ${player.aiEvaluation} 」` : (player.role ? '正在审视实力...' : '等待录入信息')}
          </span>
        </div>
